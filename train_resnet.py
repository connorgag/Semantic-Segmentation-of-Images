...
best_model_path = "best_model/resnet/best_model.pt"
...
# Apply the initial weights only to the decoder
resnet_model.deconv1.apply(init_weights)
resnet_model.deconv2.apply(init_weights)
resnet_model.deconv3.apply(init_weights)
resnet_model.deconv4.apply(init_weights)
resnet_model.deconv5.apply(init_weights)
resnet_model.classifier.apply(init_weights)
...
def train():
    ...
    for epoch in range(epochs):
        ...
        for iter, (inputs, labels) in enumerate(train_loader):
            ...
            outputs =  resnet_model(inputs) # TODO  Compute outputs. we will not need to transfer the output, it will be automatically in the same device as the model's!
...
def val(epoch=-1):
    ...
    """
    resnet_model.eval() # Put in eval mode (disables batchnorm/dropout) !
    ...
    resnet_model.train() #TURNING THE TRAIN MODE BACK ON TO ENABLE BATCHNORM/DROPOUT!!

    return np.mean(mean_iou_scores), np.mean(losses)


def modelTest(resnet_model):
    """
...
def exportModel(inputs):    
    ...
    resnet_model = ResNet(n_class=21)
    resnet_model.to(device)
    resnet_model.load_state_dict(torch.load(best_model_path, weights_only=True))
    
    resnet_model.eval() # Put in eval mode (disables batchnorm/dropout) !
    
    inputs = inputs.to(device)
    
    output_image = resnet_model(inputs)
